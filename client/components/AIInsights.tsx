...
export function AIInsights({ attendanceData, performanceData }: AIInsightsProps) {
  const generateInsights = (): Insight[] => {
    ...
    const supervisorWorkload = attendanceData.reduce((acc, record) => {
      acc[record.supervisorName] = (acc[record.supervisorName] || 0) + record.totalManhours;
      return acc;
    ...
    if (maxWorkload / minWorkload > 2) {
      insights.push({
        ...
        description: "Significant workload imbalance detected between supervisors.",
        icon: <AlertTriangle className="h-4 w-4" />
      });
    } else {
      insights.push({
    ...
    // Efficiency analysis
    const avgEfficiency = performanceData.reduce((sum, p) => sum + p.productivity, 0) / performanceData.length;
